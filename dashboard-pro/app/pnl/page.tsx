...
import { PNL_QUERY, type AssetPnl } from '../../lib/queries'

export const dynamic = 'force-dynamic'

export default async function PnlPage() {
  const { data } = await query<{ pnl: AssetPnl[] }>({ query: PNL_QUERY })
  const rows = data!.pnl
  ...
  const combinedPnl = rows.reduce((s, r) => s + r.totalPnlPct, 0)
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: 'Win Rate', value: `${overallWinRate}%` },
          {
            label: 'Combined P&L',
            value: (
              <span className={combinedPnl >= 0 ? 'text-green-400' : 'text-red-400'}>
                {combinedPnl >= 0 ? '+' : ''}
            ...
            {rows.map((r) => {
              ...
              return (
                <tr key={r.asset} className="border-b border-slate-700/40 hover:bg-slate-700/20">
                  ...
                  <td className="px-5 py-3">
                    <span className={r.avgPnlPct >= 0 ? 'text-green-400' : 'text-red-400'}>
                      {r.avgPnlPct >= 0 ? '+' : ''}
                      {r.avgPnlPct.toFixed(3)}%
                    </span>
                  </td>
                  <td className="px-5 py-3">
                    <span className={r.totalPnlPct >= 0 ? 'text-green-400' : 'text-red-400'}>
                      {r.totalPnlPct >= 0 ? '+' : ''}
