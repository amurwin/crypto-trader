...
export default function Trades() {
  ...
  function load() {
    setLoading(true)
    api.trades(limit, asset || undefined)
      .then(rows => {
        const filtered = side ? rows.filter(t => t.side === side) : rows
        setTrades(filtered)
        setError(null)
      })
  ...
  useEffect(() => { load() }, [limit, asset, side])
        ...
        <div className="bg-slate-800 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {trades.map(t => (
                <tr key={t.id} className="border-b border-slate-700/40 hover:bg-slate-700/20">
                  ...
                  <td className="px-4 py-2.5">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${t.side === 'BUY' ? 'bg-blue-900/60 text-blue-300' : 'bg-purple-900/60 text-purple-300'}`}>
                      {t.side}
                    </span>
                  </td>
              ...
              {trades.length === 0 && (
                <tr><td colSpan={8} className="px-4 py-6 text-center text-slate-500">No trades</td></tr>
              )}
            </tbody>
          </table>
        </div>
